...
export default function PaymentIntentCard({ pi, onRefund, getStatusBadge, getRefundInfo, onOpen }) {
  if (!pi) return null;
  return (
    <div className="stripe-card stripe-mt-6">
      <div className="stripe-card-header">
        <h3 className="stripe-card-title">Payment Intent</h3>
      </div>

      <div className="stripe-list">
        <div className="stripe-list-item">
          <div className="stripe-flex stripe-flex-col" style={{ width: '100%', gap: '6px' }}>
            <div className="stripe-flex stripe-justify-between stripe-items-center">
              <div className="stripe-flex stripe-items-center" style={{ gap: '8px', alignItems: 'baseline' }}>
                <span
                  ...
                  style={{ fontWeight: '600', whiteSpace: 'nowrap', cursor: onOpen ? 'pointer' : 'default' }}
                  onClick={() => onOpen && onOpen(pi)}
                >
                  {pi?.currency?.toLowerCase() === 'jpy' ? `¥${Number(pi.amount).toLocaleString('ja-JP')}` : `$${pi.amount}`} {String(pi.currency || '').toUpperCase()}
                </span>
                {(() => {
            ...
            <div className="stripe-flex stripe-justify-between stripe-items-center">
              ...
              <span className="stripe-badge stripe-badge-info" style={{ whiteSpace: 'nowrap' }}>
                {(pi.payment_method_types && pi.payment_method_types[0] === 'card') ? 'オンライン' : '店頭'}
              </span>
            </div>
